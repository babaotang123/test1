 ...
 *
 */
declare function revAppDataHandler(msg: any): any;
declare class AppMsgManager {
    static getInstance(): AppMsgManager;
    private static instance;
    constructor();
    /**
    * 接收数据
    */
    private revAppDataHandler(msg);
}
/**
 *
 */
...
declare class LN {
    ...
    /** 根据name和num得到zmovie **/
    getMovieClip(moveName: string, num: number): egret.MovieClip;
    ...
    * @param y
    */
    playZmoveLoop(key: string, num: number, x: number, y: number, scale?: number): egret.MovieClip;
    /** 停止播放动画
    */
    stopZmove(z: egret.MovieClip): void;
    /** 博放技能状态的动画
     * @param key       动画名称
     * @param num       动画图片数量
     * @param x
