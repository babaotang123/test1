...
namespace zmovie{
    ...
    public set direction(num:number){
            this._direction = num;
    ...
    public set color(c:number){
        ...
        let r:number = (c >> 16);
        let g:number = (c - (r << 16)) >> 8;
        let b:number = c - (r << 16) - (g << 8);
        ...
        let farr:any[] =  this.filters;
        if(null == farr){
            farr = [this.colorFlilter];
        }else{
            let isfind:boolean = false;
            for(let i:number = farr.length-1;i>=0;i--){
    ...
    public _direction:number;public fmObj:any;public beginFrame:number;public endFrame:number;  public parentCFOObj:any;public areaObj:any;
    public totalFrame:number;public frameTime:number;public isPlay:boolean;public currPlaylabel:string;
    public isLoop:boolean;public currArea:any[];public img_depth:number;public mcObj:any;
    public parentPlayTime:number;
    public _scale:number;public imgArr:any;public mcName:string;public libObj:any;public scaleNum:number;

    public obj:Object;// 影片剪辑不会使用这个数据，是其它程序需要使用的话预留的一个对象


    public childStop:boolean;
    public parentMaskI:number;
    private libScale:number;        private currFrame:number;        private layerArr:any[];        private isFormatScale:boolean;
    ...
    private parentMask:egret.DisplayObject;
    ...
    public isMovie():boolean{
            ...
            for(let i:number = this.numChildren-1;i>=0;i--){
                ...
                if(this.getChildAt(i) instanceof ZMovieClip){
                    const zm:ZMovieClip = this.getChildAt(i) as ZMovieClip;
                    if(zm.isMovie()){
                        return true;
                    }
                }
    ...
    public formatScale():void{
            ...
            const screenH:number = this.mcObj.screenH;
            let bl: number;
    ...
    public setTimeScale(num:number):void{
            ...
            if(!this.isPlay){
                 if(this.parent instanceof ZMovieClip){
                    const pzm:ZMovieClip = this.parent as ZMovieClip;
                    if(null != pzm){
    ...
    public getCurrLabelObj():any{
        const label_obj:any = this.getLabels();
        if(null != label_obj){
            for(const name in label_obj){
    ...
    public disposeLayerImg():void{
        if(null != this.layerArr){
            for(let l:number = this.layerArr.length-1;l>=0;l--){
                ...
                const sp: ZShap = this.layerArr[l].sp;
                if(null != sp){
                    if(null != sp.parent){
    ...
    public setMovieObject(imgArr:any,libObj:any,mcName:string,scale_:number = 1):void{
            ...
            for(let i:number = 0;i<len;i++){
                ...
                const f_arr:any[] = arr[i];
                const f_len:number = f_arr.length;
                let pcfo:any = null;
                let k:number;

                for(let j:number = 0;j<f_len;j++){
                    const cfo:any = f_arr[j];
                    let obj3d:any = null;
    ...
    public advanceTime():void{
           ...
           if(this.frameTime<now-this.parentPlayTime){
                ...
                for(let i:number = 0;i<len;i++){
                    try{
                        const l_obj:any = this.layerArr[i];
                        const frames:any[] = l_obj.frames;
                        let cfo:any;
                        let img:ZImage = null;
                        let mc:ZMovieClip = null;
                        let li:LargeImage = null;
                        let sp: ZShap = null;
                        let tf:ZTextField = null;

                        if(l_obj.img instanceof ZImage){
                            img = (l_obj.img as ZImage);
                        }
                        if (l_obj.mc instanceof ZMovieClip) {
                            mc = (l_obj.mc as ZMovieClip);
                        }
                        if(l_obj.li instanceof LargeImage){
                            li = (l_obj.li as LargeImage);
                        }
                        if(l_obj.sp instanceof ZShap){
                            sp = (l_obj.sp as ZShap);
                        }
                        if(l_obj.tf instanceof ZTextField){
                            tf = (l_obj.tf as ZTextField);
                        }
                        try{
                        ...
                        if(null == cfo.libName){
                            if(null != cfo.igd){
                                ...
                                const text:any = cfo.text;
                                const tfObj:any = text.tf;
                        ...
                        if(null != d){
                            ...
                            let mobj:any = this.mcObj.maskObj;
                            if(d.parent == this){
    ...
    private addChildToDepth(id:any):void{
        const children:number = this.numChildren;
        if(0 == children){
            this.addChild(id);
            return;
        }
        for(let i:number = children-1;i>=0;i--){
            try{
                const t_id:any = this.getChildAt(i);
                if(null != t_id){
